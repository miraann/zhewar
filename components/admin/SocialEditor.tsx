  ...
  return (
    <div className="px-4 py-6 space-y-5">
      ...
      <div className="rounded-md-lg border border-md-outline-variant bg-md-surface-container-high p-4 space-y-3">
        <p className="text-md-on-surface-variant text-xs font-semibold tracking-wider">زیادکردنی بەستەر</p>

        <div>
          <p className="text-md-on-surface text-xs font-medium mb-1.5">وێنە / ئایکۆن <span className="text-md-error">*</span></p>
          <input
          ...
          {imageUrl ? (
            <div className="flex items-center gap-3">
              <img src={imageUrl} alt="" className="w-12 h-12 rounded-md-sm object-cover border border-md-outline-variant flex-shrink-0" />
              <button onClick={() => fileRef.current?.click()} className="text-md-primary text-xs font-medium touch-manipulation">گۆڕینی وێنە</button>
              <button onClick={() => setImageUrl('')} className="text-md-on-surface-variant active:text-md-error touch-manipulation ml-auto">
                <X className="w-4 h-4" />
              </button>
            </div>
          ) : (
            <button
              ...
              disabled={uploading}
              className="w-full flex items-center justify-center gap-2 py-3 rounded-md-sm border-2 border-dashed border-md-outline bg-md-surface-container text-md-on-surface-variant text-xs touch-manipulation active:bg-md-surface-container-highest transition-colors"
            >
              {uploading ? <Loader2 className="w-4 h-4 animate-spin text-md-primary" /> : <Upload className="w-4 h-4" />}
        ...
        <div className="relative">
          <LinkIcon className="absolute start-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-md-on-surface-variant pointer-events-none" />
          <input
        ...
        <button
          ...
          className={[
            'w-full flex items-center justify-center gap-2 py-3.5 rounded-md-full font-semibold text-sm transition-all touch-manipulation',
            !imageUrl || !url.trim() || adding || uploading
              ? 'bg-md-surface-container-high text-md-on-surface-variant border border-md-outline-variant cursor-not-allowed'
              : 'bg-md-primary text-md-on-primary shadow-md-1 active:bg-md-primary/90 active:scale-[0.98]',
          ].join(' ')}
        >
      ...
      {loading && (
        <div className="space-y-2">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="h-16 rounded-md-lg bg-md-surface-container-high border border-md-outline-variant animate-pulse" />
          ))}
        </div>
      )}
      ...
      {!loading && links.length === 0 && (
        <div className="flex flex-col items-center justify-center py-12 gap-3">
          <Share2 className="w-9 h-9 text-md-outline" />
          <p className="text-md-on-surface-variant text-sm">هیچ بەستەرێک نییە</p>
        </div>
      )}
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center" onClick={() => setEditing(null)}>
          ...
          <div
            className="relative w-full md:w-[400px] bg-md-surface-container border border-md-outline-variant rounded-t-md-xl md:rounded-md-xl px-5 pt-4 pb-8 md:pb-5 shadow-md-2 space-y-4"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="w-10 h-1 rounded-full bg-md-outline-variant mx-auto md:hidden" />
            <p className="text-md-on-surface font-semibold text-sm text-center">دەستکاریکردنی بەستەر</p>
            ...
            <div>
              ...
              {editing.image_url ? (
                <div className="flex items-center gap-3">
                  <img src={editing.image_url} alt="" className="w-12 h-12 rounded-md-sm object-cover border border-md-outline-variant flex-shrink-0" />
                  <button onClick={() => editFileRef.current?.click()} className="text-md-primary text-xs font-medium touch-manipulation">
                    {editUploading ? <Loader2 className="w-3.5 h-3.5 animate-spin inline mr-1" /> : null}
                    گۆڕینی وێنە
                  </button>
                  <button onClick={() => setEditing({ ...editing, image_url: '' })} className="text-md-on-surface-variant active:text-md-error touch-manipulation ml-auto">
                    <X className="w-4 h-4" />
                  </button>
                </div>
              ) : (
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-3 rounded-md-sm border-2 border-dashed border-md-outline bg-md-surface-container-high text-md-on-surface-variant text-xs touch-manipulation"
                >
                  {editUploading ? <Loader2 className="w-4 h-4 animate-spin text-md-primary" /> : <Upload className="w-4 h-4" />}
            ...
            <div>
              <p className="text-md-on-surface text-xs font-medium mb-1.5">ناونیشان (ئارەزوومەند)</p>
              <input
            ...
            <div>
              <p className="text-md-on-surface text-xs font-medium mb-1.5">بەستەر</p>
              <div className="relative">
                <LinkIcon className="absolute start-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-md-on-surface-variant pointer-events-none" />
                <input
            ...
            {editError && <p className="text-md-error text-xs">{editError}</p>}
  ...
  return (
    ...
    >
      {link.image_url ? (
        ...
        <div className="w-full h-full bg-md-surface-container-high flex flex-col items-center justify-center gap-2">
          <Share2 className="w-8 h-8 text-md-outline" />
        </div>
      )}
